...
#' @param extraction type of data extraction
#' @param extraction_param number of points in the data extraction
#' @param symbol_set symbol set in whcih the data is coded
...
#' 
get_values <- function(incoming_list = NULL, data_summary = NULL, name_separator = '_', name_parameters = NULL,
  ...
  for(i in 1:length(data_summary)){
    ...
    #--------------------------------------------------------------------------------
    #segments-----------------------
    if(!is.null(segment_tier) && segment_position == T){
    ...
    #words--------------------------
    if(!is.null(word_tier) && word_position == T){
    ...
    for(interval in 1:interval_nmbr_segment_tier)
    {
      ...
      if(length(which('all' %in% specify_segments)) == 0 && tmp_intrvl_label != pause_marker &&
         length(which(tmp_intrvl_label %in% specify_segments)) != 0){
        ...
        if((length(which('every' %in% extraction)) == 1 && seg_dur > (extraction_param/1000)) ||
          ...
          if(contiguous_segment == 'all' || length(which(contiguous_segment %in% 'intvoc')) == 1){
            ...
            if (length(which(previous_segment %in% ipacorr(symbol_set, vowel_set = 'all')$vowels)) != 0 &
                length(which(following_segment %in% ipacorr(symbol_set, vowel_set = 'all')$vowels)) != 0)
            {
              is_intervocalic = 'y'
            }else{
              is_intervocalic = 'n'
...

            {
              ...
              if(!is.null(segment_tier) & !is.null(contiguous_segment)){
                ...
                if(contiguous_segment == 'all' || contiguous_segment == 'intvoc'){
                  ...
                  #intervocalic
                  if (length(which(tmp_previous_segment %in% ipacorr(symbol_set, vowel_set = 'all')$vowels)) != 0 &&
                      length(which(tmp_following_segment %in% ipacorr(symbol_set, vowel_set = 'all')$vowels)) != 0)
                  {
                    intervocalic = 'y'
                  }else{
                    intervocalic = 'n'
              ...
              if(!is.null(phrase_tier) && !is.null(segment_tier) & !is.null(set_contiguous_segment)){
                ...
                if(set_contiguous_segment == 'all' || set_contiguous_segment == 'intvoc'){
                  ...
                  {
                    #same phrase
                    if(length(which(tmp_previous_segment_same_phrase %in% ipacorr(symbol_set, vowel_set = 'all')$vowels)) != 0 &&
                       length(which(tmp_following_segment_same_phrase %in% ipacorr(symbol_set, vowel_set = 'all')$vowels)) != 0){
              ...
              if(!is.null(word_tier) && !is.null(segment_tier) & !is.null(set_contiguous_segment)){
                ...
                if(set_contiguous_segment == 'all' || set_contiguous_segment == 'intvoc'){
                  ...
                  {
                    #same word
                    if(length(which(tmp_previous_segment_same_word %in% ipacorr(symbol_set, vowel_set = 'all')$vowels)) != 0 &&
                       length(which(tmp_following_segment_same_word %in% ipacorr(symbol_set, vowel_set = 'all')$vowels)) != 0){
