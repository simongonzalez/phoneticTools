...
get_symbols <- function(symbol_format = 'ipa', consonant_set = 'all', vowel_set = 'all',
  ...
  all_consonant_features = c("all", "affricate", "alveolar", "anterior", "approximant", "bilabial ", "consonantal", "continuant", "coronal", "delayed_release", "dental", "distributed", "dorsal", "fricative", "glottal", "labial", "labiodental", "labiovelar", "lateral", "liquid", "nasal", "palatal", "palatoalveolar ", "plosive", "rhotic", "sibilant", "sonorant", "stop", "syllabic", "velar", "voiceless")
  
  all_vowel_features = c("all", "back", "central", "diph", "front", "high", "long", "low", "mid", "monoph", "round", "tense")
  ...
  expect_character(consonant_set, min.len = 1, max.len = length(all_consonant_features))
  if(!testSubset(consonant_set, all_consonant_features))
    stop(paste0("No format with the label specified in consonant_set. Accepted labels: ", paste0(all_consonant_features, collapse = ', '), "."))
  
  expect_character(vowel_set, min.len = 1, max.len = length(all_vowel_features))
  if(!testSubset(vowel_set, all_vowel_features))
    stop(paste0("No format with the label specified in vowel_set. Accepted labels: ", paste0(all_vowel_features, collapse = ', '), "."))
  
  if(consonant_set != 'all' & !testNull(consonant_set_manual))
    stop("Entered specific values for both consonant_set and consonant_set_manual.")
  
  if(vowel_set != 'all' & !testNull(vowel_set_manual))
    stop("Entered specific values for both vowel_set and vowel_set_manual.")
  ...
  if(length(consonant_set) == 1){
    if(consonant_set == 'all'){
      if(!is.null(consonant_set_manual)){
        consonant_vector <- consonant_vector[which(consonant_vector %in% consonant_set_manual)]
      }
    }else{
